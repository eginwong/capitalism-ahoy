...
describe('Rules -> MANAGE_PROPERTIES', () => {
  ...
  describe('manageProperties', () => {
    ...
    beforeEach(() => {
      ...
      eventBus.on(cancelEvent, cancelSpy);
    });
    ...
    it(`should emit desired ${cancelEvent} event`, () => {
      const promptStub = sinon.stub(PlayerActions, 'prompt');
      promptStub.onCall(0).returns(cancelEvent);
      eventBus.emit(inputEvent);
      expect(cancelSpy.callCount).to.equal(
        1,
        `${cancelEvent} was called ${cancelSpy.callCount} times but expected to be 1 times`
      );
    });
